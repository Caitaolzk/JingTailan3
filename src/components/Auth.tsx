  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (currentScreen === 'login') {
      ...
      if (!reg.test(password)) {
        setValidationError('密码不符合规范：至少 8 位且包含数字和字母');
        return;
      }
      try {
        const user = await auth.signInWithUsernameAndPassword(email, password);
        onLoginSuccess(user.username || email);
      } catch (err: any) {
        setValidationError('登录失败：邮箱或密码错误');
      }
    }
    ...
    // 注册：用户名注册，不发邮件
      ...
      try {
        ...
        await db.collection("users").add({
          nickname,email,title:'新进传承人',avatar:'',levelFiligree:0,levelEnamel:0,completedCount:0,followersCount:0
        });
        setSuccessMsg('注册成功，账号已生效，直接前往登录！');
        setTimeout(()=>onNavigate('login'),2000);
      } catch (err: any) {
        setValidationError('注册失败：'+err.message);
      }
    }

    // 忘记密码：不再前端填密码提交，只发链接，删除code、新密码校验
    else if (currentScreen === 'forgot_password') {
      setValidationError('');
      setSuccessMsg('');
      setValidationError('请先点击【发送验证码】，前往邮箱点开链接修改密码');
    }
  };
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              {currentScreen === 'forgot_password' && (
                <div className="flex gap-2 items-end">
                  <div className="flex-1 space-y-1.5">
                    ...
                    <input id="code" type="text" disabled placeholder="收到邮件链接后在网页改密"
                      className="w-full bg-surface-container-low border border-outline-variant rounded py-2.5 px-3 text-sm text-on-surface placeholder:text-outline-variant/50 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-colors recessed-input"/>
                  </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input id="password" type={showPassword?'text':'password'} required value={password} onChange={(e)=>setPassword(e.target.value)} placeholder="••••••••" disabled={currentScreen==='forgot_password'}
              ...
              {currentScreen !== 'login' && (
                <div className="space-y-1.5">
                  <label htmlFor="confirmPassword" className="block text-xs font-sans-manrope tracking-widest text-[#e5e2e1] uppercase flex items-center gap-1.5"><Lock className="w-3.5 h-3.5 text-[#e9c349]" /> 确认密码</label>
                  <div className="relative">
                    <input id="confirmPassword" type={showConfirmPassword?'text':'password'} required value={confirmPassword} onChange={(e)=>setConfirmPassword(e.target.value)} placeholder="请再次确认新密码" disabled={currentScreen==='forgot_password'}
                      className="w-full bg-surface-container-low border border-outline-variant rounded py-2.5 px-3 text-sm text-on-surface placeholder:text-outline-variant/30 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-colors recessed-input pr-10"/>
